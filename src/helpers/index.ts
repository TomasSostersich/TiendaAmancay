import { Size, Product, VariantProduct } from '../interfaces';
...
export const prepareProducts = (products: Product[]) => {
	return products.map(product => {
		// Agrupar las variantes por size
		const sizes = product.variants.reduce(
			(acc: Size[], variant: VariantProduct) => {
		...
		const price = Math.min(...sizes.map(item => item.price));

		// Devolver el producto formateado
		return {
			...product,
			price,
			sizes: sizes.map(({ name, size }) => ({ name, size })),
			variants: product.variants,
		};
	});
};

// Función para formatear la fecha a formato x de x de 2025
